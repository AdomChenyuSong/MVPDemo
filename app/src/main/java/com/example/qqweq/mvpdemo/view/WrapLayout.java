...
public class WrapLayout extends ViewGroup {
     ...
     * @param mr 右外边距
     * @param mb 下外边距
     * @param childHight 单个view的高
     * @param childWidth 单个view的宽
     */
    public void setData(String[] data,Context context,int textSize,int pl,int pt,int pr,int pb,int ml,int mt,int mr,int mb,int childWidth,int childHight){
        createChild(data,context,textSize, pl, pt, pr, pb, ml, mt, mr, mb,childWidth,childHight);
    }

    public void setData(List<String> data, Context context, int textSize, int pl, int pt, int pr, int pb, int ml, int mt, int mr, int mb,int childWidth,int childHight){
        String[] mydata = null;
        if(data!=null){
        ...
        setData(mydata,context, textSize,pl, pt, pr, pb, ml, mt, mr, mb,childWidth,childHight);
    }

    private void createChild(String[] data, final Context context, int textSize, int pl, int pt, int pr, int pb, int ml, int mt, int mr, int mb,int childWidth,int childHight){
        ...
        for(int i = 0;i<size;i++){
            ...
            if (style == TEXTVIEW_STYLE){
                ...
                ((TextView) btn).setTextSize(textSize);
                 ...
                 btn= LayoutInflater.from(getContext()).inflate(R.layout.btn_object,null,false);
                ((Button) btn).setText(text);
                ((TextView) btn).setTextSize(textSize);
            }
            btn.setPadding(dip2px(context, pl),dip2px(context, pt),dip2px(context, pr),dip2px(context, pb));
            MarginLayoutParams params = new MarginLayoutParams(childWidth,childHight);
            if (i%6==0){
                marginLeft=0;
            }else {
                marginLeft=ml;
            }
            params.setMargins(marginLeft, mt, mr, mb);
            ...
            btn.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    setCheckChilden(finalI1);
                    markClickListener.clickMark(finalI);
                }
            });
            //默认选择第一个
            if (i==0){
                markClickListener.clickMark(0);
                btn.setSelected(true);
            }
            this.addView(btn);
        }
    }
    private void setCheckChilden(int index){
